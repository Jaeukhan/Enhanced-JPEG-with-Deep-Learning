...
if __name__ == '__main__':
    ...
    for i, file in enumerate(lidir):
        ...
        img_list.append(img)
    # 90 img 복원
    ybit, cbbit, crbit = encoding(img_list[2], qfactor, N)
    reconimg = decoding(ybit, cbbit, crbit, qfactor, N)
    cv2.imwrite('result/' + str(qfactor) + '_' + str((2 + 1) * 30) + '.jpg', reconimg)
    del img_list[2]
    # 30(0) 150(3), 60(1),120(2)
    enc1 = Two_encoding(img_list[0], img_list[3], qfactor)
    enc2 = Two_encoding(img_list[1], img_list[2], qfactor)

    encimg1, encimg2 = enc1.encoding()
    encimg3, encimg4 = enc2.encoding()

    dec1 = Two_decoding(encimg1, encimg2, qfactor)
    dec2 = Two_decoding(encimg3, encimg4, qfactor)

    reconimg30, reconimg150 = dec1.decoding()
    reconimg60, reconimg120 = dec2.decoding()

    cv2.imwrite('result/' + str(qfactor) + '_' + str(30) + '.jpg', reconimg30)
    cv2.imwrite('result/' + str(qfactor) + '_' + str(60) + '.jpg', reconimg60)
    cv2.imwrite('result/' + str(qfactor) + '_' + str(120) + '.jpg', reconimg120)
    cv2.imwrite('result/' + str(qfactor) + '_' + str(150) + '.jpg', reconimg150)
